...
**F18 — Payment detail omits attachments (notes actually round-trip) (Important; found 2026-07-15 · scope corrected 2026-07-19).** The served detail `PaymentRecord` (`models/payment.ts:138-174`) shows no attachment metadata. **Correction (verified against `Ombor.API`):** the *note* is **not** dropped — it round-trips as `description` (create `Description` → `Payment.Notes` → served `PaymentRecordDto.Description`), so surfacing it is an **FE display fix** (read the served `description`), not BE-blocked. Only **attachments** are genuinely missing end-to-end: `PaymentAttachment` (`FileId`/`FileName`) is never populated or served and payment-create is JSON, so a file entered at creation is dropped. The BE half (multipart create + `PaymentAttachmentDto{id,name,url}` on the GET) is tracked in `../Ombor.Docs/issues-tracker.md` §12.A (F18); FE keeps no stub.
