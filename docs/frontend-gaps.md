...
| F6 | **Refund «Возврат к №N» renders blank** — served `originalTransactionNumber` dropped by `toRecord` | **Missing** (broken) | Important | `TransactionApi.ts:27-77`, `transactionColumns.tsx:41` |
| F7 | **Transaction attachments mis-shaped** — FE `{kind,size}` vs served `{contentType,sizeBytes,url}` → wrong icon, blank size, no download link | **Missing** (broken) | Important | `models/transaction.ts:40`, `cards.tsx:669` |
| F8 | **Warehouse «Движения» refund filter matches nothing** — FE collapses to one `Refund` kind; contract splits `SaleRefund`/`SupplyRefund` | **Missing** (broken) | Important | `models/warehouse.ts:58`, `WarehouseMovementsTab.tsx:144` |
...
| F12 | **Partner Telegram silently dropped** — form collects `telegram`, sent in create/update, absent from the contract | **Missing** (broken) | Important | `PartnerFormModal.tsx:331`, `PartnerPage.tsx:57` |
| F13 | **Wallet transfer over-balance guard** blocks all transfers from a negatively-balanced wallet (needs `Math.max(available,0)`). The POS supply guard + its error copy got the clamp 2026-07-17 (`NewTransactionEntry.tsx`, `TransactionSummaryCard.tsx`); the transfer modal and the `payment.form.overWallet` copy (`PaymentCreateModal.tsx:566`, negative «Доступно только…») are still unclamped | Latent | Important | `WalletTransferModal.tsx:158` |
...
**F21 — POS package-unit entry: entered pack count not persisted (Important; found 2026-07-17).** The POS cart line offers per-line package entry on packaged products (a «шт | упак» toggle, `CartLineQty.tsx`; entry converts to base units per R21 — package count × size, quantity stays whole base units). But R21 also says "The entered package count is retained on the line for audit" (`business-rules.md:51`), and the contract cannot carry it: `CreateTransactionLine` = `productId, quantity, unitPrice, discount, discountType` only (`transactions-payments.md:100-105`); the template item contract is likewise pack-blind. The flag is FE-entry-only (`CartItem.inPackages`, `useTransactionEntry.ts`) — lost on submit and on template save/load; the transaction detail shows base units. Satisfying R21's audit clause needs a pack-count field on the transaction-line write+read DTOs (and on template items for a pack-mode round-trip).
