...
9. **No system walk-in partner — a partner is always required.** The product owner dropped the system «Розничный покупатель» (canon rules 39–40, mvp-plan §8 and product-brief, which still describe it, are superseded — propose the canon edits, don't apply). Partners are uniform; a tenant gets an ordinary **default partner** at registration that can be reused for anonymous/walk-in retail sales. The POS New Sale therefore has **no default partner** — its picker starts empty and selection is required on submit.
