...
4. **Balances: color + natural-language label, no +/- signs.** E.g., green «Вам должны 1 250 000» / red «Вы должны 800 000» / neutral «Нет долга». Applies to partner balances, debt lists, wallet figures.
   - _Color-semantics note:_ green/red are reserved for money **figures** (balances, amounts). Status/lifecycle **chips** may use the full semantic palette including green/red where the state maps to good/bad (e.g. order Delivered = green, Returned/Rejected = red). A colored chip is not a money figure, so there is no collision — the reservation is about ambiguous _numbers_, not chips.
...
20. **Detail-page canonical structure.** Detail pages are assembled from shared scaffold components — never a per-module hand-rolled header, tab bar, or card:
    - **`DetailPageHeader`** — back button -> **name title** -> `⋮` kebab (shared `ActionMenu`) -> optional `primaryAction` slot (one child-event creation action per #2). **No breadcrumb.** Optional `titleExtra` slot may hold status/source chips beside the name.
    ...
    - **Title = entity name, plus optional status/source chips (`titleExtra`).** Every other reference field lives in the summary region (a `DetailCard` or the hero card), not the title. _(The Orders header also uses a `meta` slot for a «created … · customer» line — this is **provisional and under review** as possible drift from the name-only principle; do not propagate the `meta` slot to new detail pages without deciding it first.)_
