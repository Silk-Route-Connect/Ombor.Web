...
> - **XC-8 wallet-half — DONE, FE code-only pending a backend restart (2026-07-19):** backend landed `walletId` on `TransactionPaymentDto` + `PartnerLedgerEntryDto` (PR #104, merged into `redesign/issue-fixes`; contracts updated). FE wiring landed: `walletId` modeled on `PartnerLedgerEntry` (`models/partner.ts`) + `TransactionPaymentLine` (`models/transaction.ts`), and a `WalletLink` renders in the partner «Платежи» tab (`PaymentsTab.tsx`) and the transaction-detail payments card (`cards.tsx`), each falling back to plain text when `walletId` is absent. Live-verified the **fallback** (the locally-running backend build predates PR #104, so it still omits `walletId` → wallet renders as plain «Касса», no crash); restart the local backend on the merged build to see the link. No further FE change needed.
