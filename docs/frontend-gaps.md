...
**Verification basis:** `docs/openapi.json` (regenerated **2026-07-05** by commit `31277f2 (redesign/backend-alignment) — run against the real backend, sync the API contract`) + the frontend source tree (`src/models`, `src/services/api`, `src/stores`, `src/components`) + `business-rules.md` (canon) + `decision-logs.md`. Every verdict cites a route/DTO/field or a `file:line`.
...
*End of regenerated list. Items A1–A26, F1–F19, U1–U3 above are the verified state as of 2026-07-09 against the API contract and the current working tree.*
