...
> - **XC-8 wallet-half — STILL BLOCKED** (unchanged): no `walletId` on `TransactionPaymentDto`/`PartnerLedgerEntryDto`, so partner-payments can't link to the wallet. Remains a backend item.
