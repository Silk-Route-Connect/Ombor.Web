...
| `EntityFilterSelect`                                                   | Compact table filter dropdowns (warehouse, category) | Icon-led `Select` + «Все …» first option; the compact-dropdown style defined once (XC-1). Small sets read better than a typeahead |
